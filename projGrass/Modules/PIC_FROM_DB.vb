...
Module PIC_FROM_DB
    ...
    Public Function get_imgCam(ByVal Lcode As String) As Bitmap
        Try
            Dim rs As New Recordset
            rs = Conn.Execute("select CamImage from tblimgcam where LogCode = '" & Lcode & "'")
            If rs.EOF = False Then
                Dim imgByteArray() As Byte
                Try
                    imgByteArray = CType(rs("CamImage").Value, Byte())
                    Dim stream As New System.IO.MemoryStream(imgByteArray)
                    Dim bmp As New Bitmap(stream)
                    stream.Close()
                    Return bmp
    ...
    Public Function get_imgId(ByVal Lcode As String) As Bitmap
        Try
            Dim rs As New Recordset
            rs = Conn.Execute("select IdImage from tblimgid where LogCode = '" & Lcode & "'")
            If rs.EOF = False Then
                Dim imgByteArray() As Byte
                Try
                    imgByteArray = CType(rs("IdImage").Value, Byte())
                    Dim stream As New System.IO.MemoryStream(imgByteArray)
                    Dim bmp As New Bitmap(stream)
                    stream.Close()
                    Return bmp
    ...
    Public Function get_imgSigna(ByVal Lcode As String) As Bitmap
        Try
            Dim rs As New Recordset
            rs = Conn.Execute("select CamImage from tblimgsigna where LogCode = '" & Lcode & "'")
            If rs.EOF = False Then
                Dim imgByteArray() As Byte
                Try
                    imgByteArray = CType(rs("SignaImage").Value, Byte())
                    Dim stream As New System.IO.MemoryStream(imgByteArray)
                    Dim bmp As New Bitmap(stream)
                    stream.Close()
                    Return bmp
