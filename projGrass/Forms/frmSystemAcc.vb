...
Public Class frmSystemAcc
    ...
    Sub fill()
        ...
        Dim lup As Short
        rs = New Recordset
        rs = ConnLoc.Execute("select * from tblUserAcc")
    ...
    Sub find()
        ...
        Dim lup As Short
        rs = New Recordset
        rs = ConnLoc.Execute("select * from tblUserAcc where " & cboCateg.Text & " like '" & txtKeyword.Text & "%'")
        txtCnt.Text = rs.RecordCount
        If rs.EOF = False Then
            For lup = 1 To rs.RecordCount
                lstList.Refresh()
    ...
    Private Sub txtKeyword_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtKeyword.KeyDown
        If e.KeyCode = Keys.Enter Then
            Call cmdFind_Click(sender, New System.EventArgs)
        End If
    End Sub
        ...
        For Each viewlst In lstList.Items
            If viewlst.Selected = True Then
                If MsgBox("Are you sure do you want to delete this record?    ", vbYesNo + vbQuestion + vbDefaultButton2, "Delete") = vbYes Then
                    Dim rs As New Recordset
                    rs = New Recordset
                    rs = ConnLoc.Execute("delete from tblUserAcc where Username = '" & viewlst.SubItems(0).Text & "'")
    ...
    Private Sub cmdDel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDel.Click
        If conLocal() = False Then
            MsgBox("Please connect to local database!", vbExclamation, "Database Conccetion")
            frmConnDB.ShowDialog()
            Exit Sub
        End If
